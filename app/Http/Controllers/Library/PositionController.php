...
class PositionController extends Controller
{
    ...
    public function index()
    {
        return response()->json(
            LibPosition::select('id', 'position_code', 'position_desc')
                ...
                ->where('is_archived', 0) // exclude archived positions
    ...
    {
        $position = LibPosition::find($id);
        if (! $position) {
            return response()->json(['message' => 'Position not found'], 404);
        }

        $position->is_archived = 1; // mark as archived
        $position->save();

        return response()->json(['message' => 'Position archived successfully']);
    }

    public function archived()
    {
        $positions = LibPosition::where('is_archived', 1)->get();
